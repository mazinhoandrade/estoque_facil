...
export default function NewSalePage() {
  ...
  const [products, setProducts] = useState<Array<{ id: string, quantity: number, name: string, price: number, quantityMax: number }>>([])
  ...
  const searchProducts = useCallback(debounce(async (query: string) => {
    if (!query.trim()) {
      setSearchResults([])
      return
    }
  ...
  const addProduct = (product: Product) => {
    ...
    if (quantity > product.quantity) return toast({
  ...
  const handleSubmit = async () => {
    if (!clientName || products.length === 0) {
      toast({
        ...
        description: 'Por favor, preencha o nome do cliente e adicione produtos ao carrinho.',
      })
      return
    }
              ...
              {searchResults.map((product) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-600">
                    Código: {product.code} | Preço: R$ {product.price.toFixed(2)} | Estoque: {product.quantity}
                  </div>
                </div>
              ))}
            </div>
