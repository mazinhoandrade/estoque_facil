      ...
      <div className="grid-cols grid w-full gap-4 text-left text-xl lg:grid-cols-2">
        ...
        <div className="mt-4 space-y-1">
            ...
            {Products.filter((product) => product.quantity <= 4).slice(0, 10).map((product) => (
              <Card key={product.id} className="bg-primary py-2 text-center flex justify-between items-center text-white px-1"><div>{product.name}</div><div className="text-xs">{product.quantity} unidades</div></Card>
            ))}
        </div>
      </div>
    </div>
